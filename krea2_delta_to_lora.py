...
def normalize_target_base(key: str, prefix_policy: str) -> str:
    """Map checkpoint parameter key -> ComfyUI generic LoRA target base key."""
    # For matrix .weight parameters, LoRA target is module name, not parameter suffix.
    if key.endswith(".weight"):
        base = key[:-len(".weight")]
    else:
        # ComfyUI generic mapping also exposes non-.weight state-dict keys directly.
        base = key
    ...
    # Native Krea2 checkpoints use names such as blocks.0.attn.wq / first / txtfusion.
...
def main() -> int:
    ...
    with safe_open(str(path_a), framework="pt", device="cpu") as fa, safe_open(str(path_b), framework="pt", device="cpu") as fb:
        ...
        selected = [k for k in common if passes_filters(k, include_re, exclude_re)]
