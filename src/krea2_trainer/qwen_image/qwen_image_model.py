...
from krea2_trainer.modules.fp8_optimization_utils import apply_fp8_monkey_patch
from krea2_trainer.qwen_image.qwen_image_modules import get_activation
from krea2_trainer.hunyuan_model.attention import attention as hunyuan_attention

import logging

from krea2_trainer.utils.lora_utils import load_safetensors_with_lora_and_fp8
...
class QwenImageTransformer2DModel(nn.Module):  # ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    ...
    def enable_block_swap(self, blocks_to_swap: int, config: BlockSwapConfig):
        ...
        self.offloader = create_offloader("qwen-image-block", self.transformer_blocks, self.num_blocks, self.blocks_to_swap, config)
        print(
            f"QwenModel: Block swap enabled. Swapping {self.blocks_to_swap} blocks out of {self.num_blocks} blocks. "
