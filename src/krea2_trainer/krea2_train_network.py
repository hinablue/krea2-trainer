...
from krea2_trainer.utils import model_utils
...
class Krea2NetworkTrainer(NetworkTrainer):
    ...
    def handle_model_specific_args(self, args):
        ...
        # K2 fp8 supports only the scaled (dynamic) path. Treat --fp8_base as the user-facing
        # request and automatically enable --fp8_scaled, so migrated sd-scripts style configs do
        # not silently run bf16 or fail just because they lack the K2-specific companion flag.
        if args.fp8_base and not args.fp8_scaled:
            logger.info("Krea 2 fp8 requested via --fp8_base; enabling --fp8_scaled for dynamic scaled fp8 DiT loading.")
            args.fp8_scaled = True
        if args.cache_text_encoder_outputs_to_disk:
            logger.info(
                "--cache_text_encoder_outputs_to_disk accepted for sd-scripts compatibility. "
                "Krea 2 TE caches are always disk-backed *_krea2_te.safetensors files."
            )
        ...
        if args.cache_te_every_epoch and args.text_encoder is None:
            raise ValueError("--cache_te_every_epoch requires --text_encoder so Krea2 can rebuild TE cache each epoch.")
        if not 0.0 <= args.cache_te_caption_dropout_rate <= 1.0:
            raise ValueError("--cache_te_caption_dropout_rate must be between 0.0 and 1.0.")
        if args.cache_te_keep_tokens < 0:
            raise ValueError("--cache_te_keep_tokens must be 0 or greater.")

    @staticmethod
    def _caption_rng_seed(args: argparse.Namespace, epoch: int, item_key: str) -> int:
        base_seed = 0 if args.seed is None else int(args.seed)
        key_digest = hashlib.blake2b(item_key.encode("utf-8"), digest_size=8).digest()
        key_seed = int.from_bytes(key_digest, "big", signed=False)
        return (base_seed + int(args.cache_te_seed_offset) + epoch * 1_000_003 + key_seed) % (2**63)

    @staticmethod
    def _augment_caption_for_cache(caption: str, args: argparse.Namespace, epoch: int, item_key: str) -> str:
        separator = args.cache_te_caption_separator
        if separator:
            raw_tokens = caption.split(separator)
            joiner = separator + " " if separator.strip() == separator and separator != " " else separator
        ...
        keep_tokens = max(0, int(args.cache_te_keep_tokens))
        kept = tokens[:keep_tokens]
        rest = tokens[keep_tokens:]

        rng = random.Random(Krea2NetworkTrainer._caption_rng_seed(args, epoch, item_key))
        dropout_rate = float(args.cache_te_caption_dropout_rate)
        if dropout_rate > 0:
            rest = [token for token in rest if rng.random() >= dropout_rate]

        if args.cache_te_shuffle_caption and len(rest) > 1:
            rng.shuffle(rest)

        return joiner.join(kept + rest)

    def _refresh_text_encoder_cache_for_epoch(self, args, accelerator, train_dataset_group, epoch: int) -> None:
        device_arg = args.cache_te_device if args.cache_te_device is not None else str(accelerator.device)
        device = torch.device(device_arg)
        te_dtype = torch.bfloat16 if args.cache_te_dtype is None else model_utils.str_to_dtype(args.cache_te_dtype)
        previous_logging_disable = logging.root.manager.disable

        logger.debug(
            "Refreshing Krea2 text encoder cache for epoch %s on %s "
            "(shuffle=%s, dropout=%s, keep_tokens=%s)",
            epoch,
            device,
            args.cache_te_shuffle_caption,
            args.cache_te_caption_dropout_rate,
            args.cache_te_keep_tokens,
        )
        ...
        try:
            ...
            def encode(batch: list[ItemInfo]) -> None:
                augmented_prompts = [self._augment_caption_for_cache(item.caption, args, epoch, item.item_key) for item in batch]
                for item, prompt in zip(batch, augmented_prompts):
                    logger.debug("cache TE epoch %s: %s -> %s", epoch, item.item_key, prompt)

                with torch.no_grad():
                    hiddens, mask = krea2_utils.get_krea2_prompt_embeds(encoder, augmented_prompts)

                for item, hidden_i, mask_i in zip(batch, hiddens, mask):
                    valid = mask_i.bool()
                    embed_i = hidden_i[valid]
            ...
            cache_text_encoder_outputs.process_text_encoder_batches(
                args.cache_te_num_workers,
                False,  # always overwrite cache when refreshing for a new caption variant
                args.cache_te_batch_size,
                datasets,
                all_cache_files_for_dataset,
                all_cache_paths_for_dataset,
                encode,
                desc=f"Caching TE epoch {epoch}",
            )
        finally:
    ...
    def on_before_epoch(
        ...
        if not args.cache_te_every_epoch:
            return

        optimizer_eval_fn()
        accelerator.wait_for_everyone()
        if accelerator.is_main_process:
            self._refresh_text_encoder_cache_for_epoch(args, accelerator, train_dataset_group, epoch)
        accelerator.wait_for_everyone()
    ...
    def _snapshot_weights(self, model) -> dict:
        ...
        """
        live_tensors = self._named_live_tensors(model)
        return {
            k: t.detach().to("cpu", copy=True)
            for k, t in tqdm(live_tensors.items(), desc="Caching UNet", leave=False)
        }
    ...
    def _overwrite_weights(self, model, src: dict):
        ...
        """
        live_tensors = self._named_live_tensors(model)
        for k, t in tqdm(live_tensors.items(), desc="Loading UNet cache", leave=False):
            t.data.copy_(src[k])
    ...
    def _free_base_weights(self, model):
        ...
        """
        live_tensors = self._named_live_tensors(model)
        for t in tqdm(live_tensors.values(), desc="Clearing UNet", leave=False):
    ...
    def load_transformer(
        ...
        if args.fp8_scaled:
            fp8_params = sum(1 for p in model.parameters() if p.dtype in (torch.float8_e4m3fn, torch.float8_e5m2))
            fp8_buffers = sum(1 for b in model.buffers() if b.dtype in (torch.float8_e4m3fn, torch.float8_e5m2))
            scale_buffers = sum(1 for name, _ in model.named_buffers() if name.endswith(".scale_weight"))
            logger.info(
                "Krea 2 DiT fp8_scaled active for %s: %s fp8 parameter tensors, %s fp8 buffers, %s scale_weight buffers.",
                dit_path,
                fp8_params,
                fp8_buffers,
                scale_buffers,
...
def krea2_setup_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser.add_argument(
        ...
        default=None,
        help="Apply Krea2 standalone defaults for LoRA training: bf16, SDPA, shift timesteps, adamw8bit, rank/alpha 32.",
    )
    parser.add_argument(
        "--raw_dit",
        dest="dit",
        default=None,
        help="Alias for --dit; RAW Krea2 DiT checkpoint used for training.",
    )
    parser.add_argument(
        "--fp8_scaled",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Qwen3-VL-4B text encoder safetensors path (needed for sample generation and --cache_te_every_epoch)",
    )
    parser.add_argument(
        "--cache_te_every_epoch",
        "--cache-te-every-epoch",
        action="store_true",
        help="Rebuild Krea2 text encoder caches before every epoch so caption shuffle/dropout can affect cached embeddings.",
    )
    parser.add_argument(
        "--cache_te_shuffle_caption",
        "--cache-te-shuffle-caption",
        action="store_true",
        help="Shuffle caption tags when rebuilding text encoder cache. Tags before --cache_te_keep_tokens stay fixed.",
    )
    parser.add_argument(
        "--cache_te_caption_dropout_rate",
        "--cache-te-caption-dropout-rate",
        type=float,
        default=0.0,
        help="Per-tag dropout probability applied when rebuilding text encoder cache. Tags before --cache_te_keep_tokens stay fixed.",
    )
    parser.add_argument(
        "--cache_te_keep_tokens",
        "--cache-te-keep-tokens",
        type=int,
        default=0,
        help="Number of leading caption tags to preserve before shuffle/dropout during text encoder cache rebuild.",
    )
    parser.add_argument(
        "--cache_te_caption_separator",
        "--cache-te-caption-separator",
        type=str,
        default=",",
        help="Separator used to split captions into tags for cache-time shuffle/dropout. Default: comma.",
    )
    parser.add_argument(
        "--cache_te_seed_offset",
        "--cache-te-seed-offset",
        type=int,
        default=0,
        help="Extra seed offset for deterministic per-epoch cache caption augmentation.",
    )
    parser.add_argument(
        "--cache_te_batch_size",
        "--cache-te-batch-size",
        type=int,
        default=None,
        help="Batch size for in-training text encoder cache refresh. Default follows dataset cache batches.",
    )
    parser.add_argument(
        "--cache_te_num_workers",
        "--cache-te-num-workers",
        type=int,
        default=None,
        help="Dataset workers for in-training text encoder cache refresh. Default is CPU count - 1.",
    )
    parser.add_argument(
        "--cache_te_device",
        "--cache-te-device",
        type=str,
        default=None,
        help="Device for in-training text encoder cache refresh. Default: accelerator device; use cpu to avoid GPU VRAM pressure.",
    )
    parser.add_argument(
        "--cache_te_dtype",
        "--cache-te-dtype",
        type=str,
        default=None,
        help="Text encoder dtype for in-training cache refresh. Default: bfloat16.",
    )
    parser.add_argument(
        "--cache_text_encoder_outputs_to_disk",
        action="store_true",
        help="sd-scripts compatibility alias. Krea2 text encoder outputs are always disk-backed *_krea2_te.safetensors caches.",
    )
    parser.add_argument(
        "--turbo_dit",
        type=str,
...
def apply_krea2_preset(args: argparse.Namespace) -> argparse.Namespace:
    ...
    # Safe Krea2 LoRA defaults from the standalone extraction notes. These intentionally
    # mirror the recommended musubi invocation while switching the network module to the
    # renamed package namespace.
    args.sdpa = True
    ...
    args.network_module = "krea2_trainer.networks.lora_krea2"
    args.network_dim = 32
