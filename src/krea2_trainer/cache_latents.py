...
from krea2_trainer.dataset.image_video_dataset import BaseDataset, ItemInfo, save_latent_cache, ARCHITECTURE_HUNYUAN_VIDEO
from krea2_trainer.hunyuan_model.vae import load_vae
from krea2_trainer.hunyuan_model.autoencoder_kl_causal_3d import AutoencoderKLCausal3D
...
def encode_datasets(datasets: list[BaseDataset], encode: callable, args: argparse.Namespace, supports_alpha: bool = False):
    ...
    for i, dataset in enumerate(datasets):
        logger.debug(f"Encoding dataset [{i}]")
        all_latent_cache_paths = []
        for _, batch in tqdm(dataset.retrieve_latent_cache_batches(num_workers), desc=f"Caching latents [{i}]", leave=False):
