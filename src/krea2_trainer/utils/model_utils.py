...
def should_mark_cudagraph_step(args: argparse.Namespace) -> bool:
    """Return whether this run needs explicit CUDA Graph iteration boundaries.

    Inductor may capture compiled blocks with CUDA Graphs. A checkpointed
    training step has a forward/backward lifetime that the compiler cannot
    always infer, so the training loop must explicitly begin each dataloader
    iteration before invoking the compiled model. The API is unavailable on
    older PyTorch builds and is unnecessary when CUDA Graphs are disabled.
    """

    compiler = getattr(torch, "compiler", None)
    return bool(
        getattr(args, "compile", False)
        and getattr(args, "compile_backend", "inductor").lower() == "inductor"
        and getattr(args, "compile_mode", "default") != "max-autotune-no-cudagraphs"
        and compiler is not None
        and hasattr(compiler, "cudagraph_mark_step_begin")
    )
