...
from krea2_trainer.modules.custom_offloading_utils import BlockSwapConfig
from krea2_trainer.modules.lr_schedulers import RexLR
from krea2_trainer.modules.scheduling_flow_match_discrete import FlowMatchDiscreteScheduler
import krea2_trainer.networks.lora as lora_module
from krea2_trainer.dataset.config_utils import BlueprintGenerator, ConfigSanitizer
from krea2_trainer.hv_generate_video import save_images_grid, save_videos_grid

import logging

from krea2_trainer.utils import huggingface_utils, model_utils, train_utils, sai_model_spec
...
from krea2_trainer.training.accelerator_setup import (
...
def parse_cli_literal(value: str):
    """Parse CLI key=value literals while accepting common shell-friendly forms.

    Upstream sd-scripts expects Python literals for --optimizer_args and
    --lr_scheduler_args, so string/bool values normally need awkward quoting such
    as `flag=True` or `mode="'bf16_sr'"`.  For a standalone trainer launcher, be
    more permissive: accept JSON/TOML-ish lowercase booleans/null and fall back
    to the raw string when the value is an unquoted name.
    """
...
def parse_cli_key_value_args(args: list[str] | None) -> dict[str, object]:
    kwargs = {}
    if args is not None and len(args) > 0:
        for arg in args:
            key, value = arg.split("=", 1)
            kwargs[key] = parse_cli_literal(value)
    return kwargs
...
def _flush_raw_log_handlers(raw_log_file: str) -> None:
    root_logger = logging.getLogger()
    for handler in root_logger.handlers:
        if getattr(handler, "_krea2_raw_log_file", None) == raw_log_file:
            handler.flush()


def _upload_raw_logs_to_wandb(args: argparse.Namespace, accelerator: Accelerator) -> None:
    raw_log_file = getattr(args, "raw_log_file", None)
    if not raw_log_file or not accelerator.is_main_process or not os.path.exists(raw_log_file):
        return
    ...
    _flush_raw_log_handlers(raw_log_file)

    try:
        import wandb

        artifact_name = "krea2-trainer-raw-logs"
        artifact = wandb.Artifact(artifact_name, type="training-log", description="Raw Python training logs from krea2-trainer")
        artifact.add_file(raw_log_file, name=os.path.basename(raw_log_file))
        wandb_tracker.tracker.log_artifact(artifact)
        logger.info(f"uploaded raw log file to WandB: {raw_log_file}")
    except Exception as e:
        logger.warning(f"failed to upload raw log file to WandB: {e}")
...
class NetworkTrainer:
    ...
    def get_optimizer(self, args, trainable_params: list[torch.nn.Parameter]) -> tuple[str, str, torch.optim.Optimizer]:
        ...
        optimizer_kwargs = parse_cli_key_value_args(args.optimizer_args)
        ...
        elif optimizer_type in ADV_OPTIMIZER_ALIASES:
            try:
                adv_optim = importlib.import_module("adv_optm.optim")
            except ImportError as e:
                raise ImportError(
                    "Advanced Optimizers is required for optimizer_type "
                    f"{args.optimizer_type!r}. Install the PyPI package with: pip install adv_optm "
                    "(the import package is adv_optm)."
                ) from e
            optimizer_class_name = ADV_OPTIMIZER_ALIASES[optimizer_type]
            optimizer_class = getattr(adv_optim, optimizer_class_name)
            logger.info(f"use Advanced Optimizers {optimizer_class_name} optimizer | {optimizer_kwargs}")
            optimizer = optimizer_class(trainable_params, lr=lr, **optimizer_kwargs)
    ...
    def get_lr_scheduler(self, args, optimizer: torch.optim.Optimizer, num_processes: int):
        ...
        lr_scheduler_kwargs = parse_cli_key_value_args(args.lr_scheduler_args)
    ...
    def on_before_epoch(
        ...
        """Called before the dataloader for a new epoch is consumed.

        Subclasses can use this hook to refresh epoch-scoped external resources,
        such as text-encoder caches, before worker processes start reading the
        next epoch's batches.
        """
        pass
    ...
    def _build_network(self, args, accelerator, transformer, vae, weight_dtype):
        ...
        # Allow short paths like `--network_module networks.lora` by putting the krea2_trainer
        # package dir on sys.path. __file__ is under krea2_trainer/training/, so step up one level.
    ...
    def _build_optimizer_and_dataloader(self, args, accelerator, network, train_dataset_group, collator, transformer):
        ...
        if args.persistent_data_loader_workers and not persistent_workers:
            logger.warning("Ignoring --persistent_data_loader_workers because max_data_loader_n_workers is 0")
    ...
    def _run_training_loop(
        ...
        for epoch in range(epoch_to_start, num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                if mark_cudagraph_step:
                    ...
                    # so CUDA Graphs does not reuse their storage prematurely.
            ...
            if len(accelerator.trackers) > 0:
                ...
                # Tracker steps must remain monotonic.  `epoch + 1` is lower
                # than the already-recorded per-step value after the first
                # epoch, causing W&B to discard the epoch metric.
                accelerator.log(logs, step=global_step)
        ...
        _upload_raw_logs_to_wandb(args, accelerator)
